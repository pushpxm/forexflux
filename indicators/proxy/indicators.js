...
const sma_inc = async (data) => {
  ...
  const results = await sma_async([d1], [14]);
  const d2 = results[0];
...
const sma_inc2 = async (data) => {
  const d1 = data.map((d) => d.close);
  const results = await sma_async([d1], [50]);
  const d2 = results[0];
  const diff = data.length - d2.length;
  const emptyArray = [...new Array(diff)].map((d) => '');
  const d3 = [...emptyArray, ...d2];
  data = data.map((d, i) => ({ ...d, sma_inc2: d3[i] }));
  return data;
};

module.exports = { sma_inc,sma_inc2 }; 
